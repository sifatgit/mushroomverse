...
@endif
                        ...
                        <form action="{{route('order.place')}}" method="POST" enctype="multipart/form-data" class="needs-validation" >
                            @csrf
                            <div class="row">
                            ...
                            <hr class="mb-1"> 
    @php $total_items = App\Models\Cart::totalItems(); @endphp
                        <input type="hidden" name="transaction_id" value="31wr5344w35">
                        <input type="hidden" name="total_items" value="{{$total_items}}">
                        
                        @php $total = 0; @endphp
                        @foreach($carts as $cart)
                        @php $sub_total = $cart->product_price*$cart->product_quantity; $total+= $sub_total; @endphp
                        @endforeach
                    ...
                    <div class="row">
                                ...
                                <div class="rounded p-2 bg-light">
                                   ...
                                   @foreach($carts as $cart)
                                    ...
                                    <div class="media mb-2">
                                        <div class="media-body"> <a href="{{route('product.view',$cart->product->id)}}"> {{$cart->product_title}}</a>
                                            <div class="small text-muted">Price: Tk {{$cart->product_price}} <span class="mx-2">|</span> Qty: {{$cart->product_quantity}} <span class="mx-2">|</span> @php $total = $cart->product_price*$cart->product_quantity @endphp Total: Tk {{$total}}</div>
                                            @php $sub_total+= $total;  @endphp 
                                        </div>
                                    </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="col-md-12 col-lg-12">
                            <div class="order-box">
                                ...
                                <div class="d-flex">
                                    <h4>Sub Total</h4>
                                    <div class="ml-auto font-weight-bold"> Tk {{$sub_total}} </div>
                                </div>
                                <hr class="my-1">
                                ...
                                <div class="d-flex gr-total">
                                    <h5>Grand Total</h5>
                                    @php $grand_total =  $sub_total + 60 @endphp
                                    <div class="ml-auto h5">Tk {{$grand_total}} </div>
                                </div>
                                <hr>
                            </div>
                        </div>
